            ...
            <div class="layui-col-md2">
                ...
                <div class="video-style" id="dvList" @scroll="onScroll">
                    <div class="layui-form-item" v-for="item,id in dvList"
                         @click="queryDvConfig(item)">
                        <div class="layui-input-block video-list">
                            <div class="video-img">
                                <img src="/static/img/timg.jpg" alt="" height="100%" width="100%">
                            </div>
    ...
    <div v-show="data_info">
        <div class="layui-row layui-col-space20 x-body" style="margin: 10px;height: calc(100% - 40px) !important;">
            <div class="layui-col-md4" style="height: 100%;margin-bottom: 20px;padding-right: 0px">
                <div class="layui-row">
                    <div class="layui-col-md12 item-info" style="height: 33.33% !important;" v-if="department_id==1">
                    ...
                    <div class="layui-col-md12 item-info" style="height: 33.33% !important;" v-else>
                    ...
                    <div class="layui-col-md12 item-info" style="height: 33.33% !important;">
                        <div class="source-list">事件列表</div>
                        <div class="jindu layui-form" id="eventList" style="overflow-y: hidden">
                            <div class="layui-form-item event-item" v-for="vo,id in DynamicEventXJs">
                                <label>@{{ vo.CameraName }}&nbsp;&nbsp;&nbsp;&nbsp;@{{vo.EventType }}</label><br>
                                <label>@{{ vo.CreatedTime }}</label>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    new Vue({
        ...
        methods: {
            ...
            getRegion: function () {
                this.$http.post("/admin/ievent/region", {type: this.type}, {emulateJSON: true})
                    .then(function (res) {
                            if (res.body.code == 0) {
                                var that = this;
            ...
            queryDetail: function (id, bool) {
                this.$http.get('/admin/ievent/show/' + id, {emulateJSON: true})
                    .then(function (res) {
                            if (res.body.code == 0) {
            ...
            getNavList: function (page, num) {
                ...
                this.$http.post('/admin/ievent', data, {emulateJSON: true})
                    .then(function (res) {
                        if (res.body.code == 0) {
                            this.isTrue = true;
            ...
            putReport: function (id) {
                this.$http.post("/admin/ievent/report", {
                    "id": id,
            ...
            initCanvas: function () {
                //各省事件top10
                var EventTop10 = JSON.parse(window.localStorage.getItem('iEventTop10'));
                ...
                //作业违规监管最新事件
                this.DynamicEventXJs = JSON.parse(window.localStorage.getItem('iDynamicEventXJs'));
                //历史数据
                var lineData = [];
                if (JSON.parse(window.localStorage.getItem('iTimeETypeCount')))
                    lineData = JSON.parse(window.localStorage.getItem('iTimeETypeCount'))[4].TimeEventTypes;
                var region = [];
                var barData = [];
                var lineSeries = [];
                var colorList = ['#F6A7F0', '#FF9660', '#33CBFD', '#FF5F5B', '#6797FF', '#5FB878', '#54b6ff', '#787878', '#5FB878'];
                var EventTypes = [];
                if (JSON.parse(window.localStorage.getItem('iEventTypes')))
                    EventTypes = JSON.parse(window.localStorage.getItem('iEventTypes'));
                var dateData = [];
                if (Object.values(this.region).length > 0) {
                    for (var i in this.region) {
                        //柱状图Y轴数据
                        for (var j in EventTypes) {
                            if (Object.values(this.region)[i] === EventTypes[j].Name) {
                ...
                line.style.width = $(document.body).width() >= 992 ? ($(document.body).width() - 60) * 66.66 / 100 - 50 + 'px' : $(document.body).width() - 120 + 'px';
                ...
                // 地图
                // 事件总数
                var eventCount = window.localStorage.getItem('iAllNums');
                this.IMapsInfo = JSON.parse(window.localStorage.getItem('iIMapsInfo'));
            ...
            getMessage: function (msg) {
                ...
                if (sData.AllNums) window.localStorage.setItem('iAllNums', sData.AllNums);
                if (sData.DynamicEventXJs) window.localStorage.setItem('iDynamicEventXJs', JSON.stringify(sData.DynamicEventXJs));
                if (sData.EventTop10) window.localStorage.setItem('iEventTop10', JSON.stringify(sData.EventTop10));
                if (sData.EventTypes) window.localStorage.setItem('iEventTypes', JSON.stringify(sData.EventTypes));
                if (sData.IMapsInfo) window.localStorage.setItem('iIMapsInfo', JSON.stringify(sData.IMapsInfo));
                if (sData.TimeETypeCount) window.localStorage.setItem('iTimeETypeCount', JSON.stringify(sData.TimeETypeCount));
                if (sData.XJNums) window.localStorage.setItem('iXJNums', sData.XJNums);
                if (sData.ZNNums) window.localStorage.setItem('iZNNums', sData.ZNNums);
                //视频巡检最新事件
                this.DynamicEventXJs = JSON.parse(window.localStorage.getItem('iDynamicEventXJs'));
            },
            send: function () {
