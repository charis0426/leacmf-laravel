...
<div id="app" class="layui-fluid" style="display: none;">
                ...
                <div class="layui-col-md12">
                    <table class="layui-table  text-center layui-form" lay-filter="demo" lay-size="sm">
                        ...
                        <tr>
                            ...
                            <th>
                                <div class="text-left">重点对象名称</div>
                            </th>
                            <th>
                                <div class="text-left">对象类型</div>
                            </th>
                            <th>
                                <div class="text-left">添加事由</div>
                            </th>
                            <th class="text-left">
                                添加时间
                            </th>
                        ...
                        <tr v-for="(vo,id) in formData">
                            ...
                            <td>
                                <div class="text-left" v-text="map[vo['type']]"></div>
                            </td>
                            <td>
                                <div class="text-left" v-text="vo['cause']"></div>
                            </td>
                            <td v-text="vo['created_at']"></td>
                            <td>
                                <button class="layui-btn layui-btn-xs layui-btn-normal" title=""
                                        @click="DvDetail(vo['type'],vo['pid'],vo['name'],vo['id'])" v-if="vo['type'] != 1">资源
                                </button>
                                <button class="layui-btn layui-btn-xs" title=""
                                        @click="queryDetail(vo['id'],vo['type'])">详情
                                </button>
        ...
        <div class="layui-row main main-new">
            <div class="layui-col-md2" style="height: 100%;top:5px;background: #eeeeee;">
                ...
                <div class="dv_detail" v-show="dv_detail">
                    <div class="out_list" v-for="vo in dvList" @click="queryDvConfig(vo.cameraid,vo.name)" :title="vo.name">
                        <div class="out-div">
                 ...
                 <div id="addLoop"  value='' style="height: 20px;
                                               position: absolute;
                                               bottom: 10px;
                                               width: 100%;">
                    <button style="width:100%" class="layui-btn layui-btn-sm" @click="addLoop()" title="加入轮播列表">加入轮播列表
                     </button>
                 </div>
            </div>
    ...
    <div class="info-detail" v-if="add_point">
        <div class="layui-row top">
        ...
        <div class="layui-row main layui-form" lay-filter="ob_choose">
            <div class="layui-col-md8">
                <div class="layui-form-item">
                    <label class="layui-form-label">对象类型</label>
                    <div class="layui-input-block">
                        <input type="radio" lay-filter="add_type" name="sex" value="2" title="转运中心" checked>
                        <input type="radio" lay-filter="add_type" name="sex" value="3" title="网点">
                        <input type="radio" lay-filter="add_type" name="sex" value="1" title="企业">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">对象名称</label>
                    <div class="layui-input-block">
                        <select id="ob_type" value="" lay-filter="ob_type" lay-verify="required">
                            <option value="">请选择对象</option>
                            <option :value="vo.id" v-for="vo in pointList">@{{ vo.name }}</option>

                        </select>
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">添加事由</label>
                    <div class="layui-input-block">
                        <input type="text" name="cause" id="cause" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <div class="layui-input-block">
                        ...
                        <button type="button" class="layui-btn layui-btn-sm" @click="subAddPoint()">确定</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    new Vue({
        ...
        methods: {
            ...
            addLoop: function () {
                       var url = '/admin/point/device/add';
                                       this.$http.post(url, {
                                            "id": this.object_id,
                                       ...
                                       .then(function (res) {
                                               if (res.body.code == 0) {
                                                var str = lea.msg(this.cameraname+'添加至轮播列表成功') ;
                                                   layer.msg(str);
                                               } else {
            ...
            DvDetail: function (type, id, name,oid) {
            this.object_id = oid;
                if (type == 2) {
                    var url = "/admin/transportation/device/" + id;
                } else if (type == 3) {
                    var url = "/admin/dot/device/" + id;
                } else {
                    layer.msg("参数非法");
                    return;
                }
            ...
            queryDetail: function (id, type) {
                this.detail_id = type;
                this.$http.get('/admin/point/object/show/' + id, {emulateJSON: true})
                    .then(function (res) {
                            if (res.body.code == 0) {
            ...
            delData: function (id) {
                ...
                layer.confirm("确认要删除吗，删除后不能恢复", {title: "删除确认"}, function (index) {
                    //删除单个数据
                    that.$http.get('/admin/point/object/delete/' + id, {emulateJSON: true})
                        .then(function (res) {
                                if (res.body.code == 0) {
                                    layer.msg("删除成功");
            ...
            batchDel: function () {
                ...
                layer.confirm("确认要删除吗，删除后不能恢复", {title: "删除确认"}, function () {
                    that.$http.post('/admin/point/object/batchdelete', {"ids": noList}, {emulateJSON: true})
                        .then(function (res) {
                            if (res.body.code == 0) {
            ...
            //查询设备配置
            queryDvConfig: function (cameraid,name) {
